...
def char_process(user_name, user_id, context):
    ...
    if check_message.method == "add":
        if login_user_id in attributes_cache and attributes_cache[login_user_id] is not None:
            ...
            temp=attributes_cache[login_user_id].save_to_db()
            if temp is not None:
                return temp
            else:
                return "角色已加入"
        ...
        temp=character.Character.load_from_db(check_message.parameters[0], login_user_id)
        if temp is not None:
            attributes_cache[login_user_id]=temp
            return attributes_cache[login_user_id].print_character_info()
        else:
            return "不存在指定角色"
    elif check_message.method == "edit":
        if login_user_id in attributes_cache and attributes_cache[login_user_id] is not None:
            temp=attributes_cache[login_user_id].update_attribute(check_message.parameters[0], check_message.parameters[1])
            if temp is not None:
                return temp
            else:
                return attributes_cache[login_user_id].print_character_info()
        ...
        temp=character.Character.query_by_owner(login_user_id)
        if temp is not  None:
            reply_str=f"\n{'_'*13}\n"
            for i in temp:
                reply_str+=f'{i:^13}\n'
